...
def _write_legacy_shard(path: Path, tokens: list):
    """Write a legacy shard (8-byte header, uint16)."""
...
def test_shard_dataset_basic(tmp_path):
    ...
    assert len(ds) > 0
    ids, labels = ds[0]
    assert ids.shape[0] == 11  # seq_len + 1
...
def test_shard_dataset_unknown_dtype_flag(tmp_path):
    ...
    with open(tmp_path / "train_shard_0000.bin", "wb") as f:
        f.write(struct.pack("<Q", len(tokens)))
        f.write(struct.pack("<H", 99))  # unknown flag
        f.write(arr.tobytes())
...
def test_shard_dataset_getitem_wrap(tmp_path):
    ...
    if len(ds3) > 0:
        ids, _ = ds3[0]  # will wrap since seq_len+1=31 > 25 total tokens
...
def test_build_optimizer_with_lr_base():
    """Covers the lr_base param group path including frozen param skip (line 388)."""
    from scripts.train import build_optimizer
...
def test_init_wandb_import_error():
    ...
    with patch("scripts.train.is_main_process", return_value=True):
        with patch.dict("sys.modules", {"wandb": None}):
            init_wandb(cfg)  # should not raise


def test_init_wandb_success():
    """Covers line 493: run_url is None → 'WandB initialized.' print."""
    from scripts.train import init_wandb
...
def test_init_wandb_with_entity():
    """Covers lines 433-437: entity branch, and line 486: run_url truthy."""
    from scripts.train import init_wandb
    from omegaconf import OmegaConf
...
def test_init_wandb_exception():
    """Covers the except Exception branch in init_wandb."""
    from scripts.train import init_wandb
    from omegaconf import OmegaConf
...
def test_shard_dataset_cross_shard_boundary(tmp_path):
    ...
    ds = ShardDataset(tmp_path, "train", seq_len=10)
    assert len(ds) > 0
    # Access a sequence that crosses the shard boundary
...
def test_log_wandb_import_error():
    ...
    with patch("scripts.train.is_main_process", return_value=True):
        orig = sys.modules.get("wandb")
        sys.modules["wandb"] = None  # None causes ImportError on `import wandb`
        try:
            log_wandb({"loss": 1.0})  # should not raise
        finally:
            # Always restore — critical so subsequent tests see real wandb
            if orig is None:
                sys.modules.pop("wandb", None)
            else:
                sys.modules["wandb"] = orig
        # Verify wandb is restored
...
def test_build_optimizer_lr_base_no_matching_params():
    ...
    model = torch.nn.Linear(4, 4)  # no shared_fc_weight
    cfg = MagicMock()
