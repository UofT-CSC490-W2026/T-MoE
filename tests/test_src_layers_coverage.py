"""Tests for src/layers/ — base and lora_moe coverage gaps."""
import pytest
import torch
...
def test_base_moe_layer_get_cached_metrics_no_tracker():
    ...
    layer._last_routing_weights = torch.randn(8, 4)
    layer._last_routing_indices = torch.randint(0, 4, (8, 2))
    layer.router = MagicMock(spec=[])  # no metrics_tracker attr
    assert layer.get_cached_metrics() is None
...
def _make_gptneo_moe(hidden=64, num_experts=4, top_k=2):
    ...
    router_cfg = MetabolicRouterConfig(hidden_dim=hidden, num_experts=num_experts, top_k=top_k)
    router = MetabolicRouter(router_cfg)
    return LoRAMoELayer.from_pretrained_mlp(mlp=mlp, router=router, lora_config=cfg, num_experts=num_experts)
...
def test_lora_moe_layer_step():
    ...
    x = torch.randn(2, 4, 64)
    moe(x)
    moe.step()  # should call router.step()
...
def test_lora_moe_layer_with_shared_base_lora():
    ...
    cfg = LoRAConfig(hidden_dim=hidden, rank=4, alpha=8, shared_base_rank=2, shared_base_alpha=2.0)
    router_cfg = MetabolicRouterConfig(hidden_dim=hidden, num_experts=4, top_k=2)
    router = MetabolicRouter(router_cfg)
    moe = LoRAMoELayer.from_pretrained_mlp(mlp=mlp, router=router, lora_config=cfg, num_experts=4)
...
def test_lora_moe_layer_router_no_record_usage_param():
    """Test with a router that doesn't accept record_usage param."""
    from src.experts.lora import LoRAConfig
    from src.layers.lora_moe import LoRAMoELayer
...
# ── LoRAMoELayer additional branches ──────────────────────────────────────────

def test_lora_moe_layer_with_qwen2_shared_base():
    """Test LoRAMoELayer with Qwen2 expert and shared_base_rank > 0."""
    from src.experts.lora import LoRAConfig
    from src.layers.lora_moe import LoRAMoELayer
    from src.routers.metabolic import MetabolicRouter
    from src.configs.router import MetabolicRouterConfig
    ...
    cfg = LoRAConfig(hidden_dim=hidden, intermediate_dim=intermediate,
                     rank=4, alpha=8, shared_base_rank=2, shared_base_alpha=2.0)
    router_cfg = MetabolicRouterConfig(hidden_dim=hidden, num_experts=4, top_k=2)
    router = MetabolicRouter(router_cfg)
    moe = LoRAMoELayer.from_pretrained_mlp(
        mlp=mlp, router=router, lora_config=cfg,
        num_experts=4, expert_type=ExpertType.QWEN2_LORA
    )
    x = torch.randn(2, 4, hidden)
    out = moe(x)
    assert out.shape == (2, 4, hidden)
...
def test_lora_moe_layer_get_cached_metrics_with_lora_norms():
    """Test get_cached_metrics computes lora_delta_norm_per_expert."""
    moe = _make_gptneo_moe()
    x = torch.randn(2, 4, 64)
    moe(x)
    metrics = moe.get_cached_metrics()
...
# ── LoRAMoELayer missing branches ─────────────────────────────────────────────

def test_lora_moe_layer_init_shared_base_lora_out_proj_none():
    """_init_shared_base_lora returns early when out_proj is None."""
    from src.experts.lora import LoRAConfig
    from src.layers.lora_moe import LoRAMoELayer
    from src.routers.metabolic import MetabolicRouter
    from src.configs.router import MetabolicRouterConfig
    ...
    # expert has no c_proj or down_proj → out_proj is None → early return
    layer._init_shared_base_lora()
    assert layer.shared_proj_lora is None


def test_lora_moe_layer_get_cached_metrics_no_weights():
    """get_cached_metrics returns None when _last_routing_weights is None."""
    moe = _make_gptneo_moe()
    moe._last_routing_weights = None
...
def test_lora_moe_layer_get_cached_metrics_indices_none():
    """get_cached_metrics handles _last_routing_indices=None (dense router)."""
    moe = _make_gptneo_moe()
    x = torch.randn(2, 4, 64)
    moe(x)  # populates cache
...
def test_lora_moe_layer_forward_metrics_none_indices():
    """forward with return_metrics=True when indices is None (dense router)."""
    moe = _make_gptneo_moe()
    x = torch.randn(2, 4, 64)
    out, metrics = moe(x, return_metrics=True)
    assert "weights" in metrics
    # Dense router returns None indices — "indices" key should not be in metrics
    # (or if it is, it should be None-derived)
    assert out.shape == (2, 4, 64)


def test_lora_moe_layer_forward_all_experts_zero_weight():
    """forward handles the case where mask.any() is False for some experts."""
    from src.experts.lora import LoRAConfig
    from src.layers.lora_moe import LoRAMoELayer
    ...
    class ZeroWeightRouter(BaseRouter):
        ...
        def forward(self, x, return_metrics=False, **kw):
            ...
            weights[:, 0] = 1.0  # only expert 0 gets weight
...
# ── LoRAMoELayer shared_proj_lora (Qwen2 path) ───────────────────────────────

def test_lora_moe_layer_shared_proj_lora_gptneo():
    """shared_proj_lora with GPT-Neo expert triggers the c_fc dtype branch."""
    from src.experts.lora import LoRAConfig
    from src.layers.lora_moe import LoRAMoELayer
    from src.routers.metabolic import MetabolicRouter
    from src.configs.router import MetabolicRouterConfig
    ...
    cfg = LoRAConfig(hidden_dim=64, rank=4, alpha=8, shared_base_rank=4, shared_base_alpha=4.0)
    router_cfg = MetabolicRouterConfig(hidden_dim=64, num_experts=2, top_k=1)
    router = MetabolicRouter(router_cfg)
    moe = LoRAMoELayer.from_pretrained_mlp(mlp=mlp, router=router, lora_config=cfg, num_experts=2)
...
def test_lora_moe_layer_forward_return_metrics_with_indices():
    """forward with return_metrics=True when router returns non-None indices."""
    from src.experts.lora import LoRAConfig
    from src.layers.lora_moe import LoRAMoELayer
    from src.routers.base import BaseRouter
...
def test_lora_moe_layer_get_cached_metrics_with_indices():
    """get_cached_metrics includes 'indices' key when _last_routing_indices is not None."""
    moe = _make_gptneo_moe()
    x = torch.randn(2, 4, 64)
    moe(x)
    # Manually set a non-None indices to trigger the branch
