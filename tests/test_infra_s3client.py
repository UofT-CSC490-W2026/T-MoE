...
def _fresh_client():
    ...
    # Replace the internal _client with our mock_s3 for assertions
...
def test_upload_file_no_progress(tmp_path):
    """Covers show_progress=False path."""
    client, mock_s3 = _fresh_client()
    f = tmp_path / "data.txt"
    f.write_text("hello")
...
def test_upload_file_with_metadata_and_content_type(tmp_path):
    """Covers lines 97, 99: metadata and content_type extra_args paths."""
    client, mock_s3 = _fresh_client()
    f = tmp_path / "data.txt"
    f.write_text("hello")
...
def test_s3client_init_with_endpoint_url():
    """Covers line 49: endpoint_url branch."""
    mock_boto3 = MagicMock()
    mock_session = MagicMock()
    mock_s3 = MagicMock()
    mock_sts = MagicMock()
