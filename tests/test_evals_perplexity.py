"""Tests for evals/perplexity.py — mocked model/tokenizer."""
import math
import pytest
import torch
from unittest.mock import MagicMock, patch

VOCAB = 256  # large enough that token IDs never exceed it in tests


# ---------------------------------------------------------------------------
# Pure utility functions (no model needed)
# ---------------------------------------------------------------------------
...
def test_autocast_context_cpu():
    ...
    with ctx:
        pass  # nullcontext — should not raise


def test_autocast_context_cuda():
    from evals.perplexity import _autocast_context
    # Just verify it returns something context-manager-like without a real GPU
...
def test_summarize_language_model_metrics_with_bpb():
    from evals.perplexity import summarize_language_model_metrics
    m = summarize_language_model_metrics(total_nll=10.0, total_tokens=10, total_bytes=100)
    assert "bpb" in m
...
def test_tokenize_worker_type_error_fallback():
    """Covers the TypeError fallback path in _tokenize_worker."""
    import queue
    from evals.perplexity import _tokenize_worker
...
def _make_window(seq_len=10):
    ...
    ids = torch.zeros(1, seq_len, dtype=torch.long)  # all zeros — safe for any vocab size
...
def test_run_batched_forward_same_length():
    ...
    assert losses.shape[0] == 9  # seq_len - 1
...
def test_run_batched_forward_different_lengths():
    ...
    w2 = _make_window(12)
    model = MagicMock()
    # Model returns logits for max_len=12
...
def test_compute_document_nll_short_seq():
    ...
    model = MagicMock()
    ids = torch.zeros(1, 1, dtype=torch.long)
    nll, tokens = compute_document_nll(model, ids, stride=512, max_length=1024, device="cpu")
    assert nll == 0.0
    assert tokens == 0
...
def test_compute_document_nll_basic():
    ...
    ids = torch.zeros(1, 10, dtype=torch.long)  # all zeros — safe for any vocab
    nll, tokens = compute_document_nll(model, ids, stride=10, max_length=10, device="cpu")
    assert tokens > 0
    assert nll > 0
...
def test_compute_document_nll_invalid_shape():
    ...
    with pytest.raises(ValueError, match="shape"):
        compute_document_nll(model, torch.zeros(2, 5), stride=5, max_length=5, device="cpu")
...
def test_compute_document_nll_invalid_stride():
    ...
    with pytest.raises(ValueError, match="stride"):
        compute_document_nll(model, torch.zeros(1, 5), stride=0, max_length=5, device="cpu")
...
def test_compute_document_nll_invalid_max_length():
    ...
    with pytest.raises(ValueError, match="max_length"):
        compute_document_nll(model, torch.zeros(1, 5), stride=5, max_length=1, device="cpu")
...
def test_evaluate_text_documents_basic():
    ...
    result = evaluate_text_documents(
        model_fn, tokenizer, ["hello world " * 10],
        stride=10, max_length=20, device="cpu",
        include_bpb=True, batch_size=1,
    )
...
def test_evaluate_text_documents_no_tokens():
    ...
    with pytest.raises(ValueError, match="No tokens scored"):
        evaluate_text_documents(
            model, tokenizer, ["x"],
            stride=512, max_length=512, device="cpu",
            include_bpb=False, batch_size=1,
        )
...
def test_load_dataset_texts_missing_column():
    ...
    spec = {"hf_path": "fake/path", "split": "test", "streaming": True, "text_column": "text"}
    with patch.dict("sys.modules", {"datasets": mock_datasets}):
...
def _write_val_shard(path, tokens):
    ...
    with open(path, "wb") as f:
        ...
        f.write(struct.pack("<H", 0))  # dtype_flag=0 (uint16)
        f.write(arr.tobytes())
...
def test_evaluate_token_shards_no_shards(tmp_path):
    ...
    with pytest.raises(FileNotFoundError, match="No val_shard"):
        evaluate_token_shards(
            model, tmp_path,
            stride=4, max_length=8, device="cpu",
            autocast_dtype=torch.float32, batch_size=2,
        )
...
def test_evaluate_token_shards_basic(tmp_path):
    ...
    result = evaluate_token_shards(
        model_fn, tmp_path,
        stride=8, max_length=8, device="cpu",
...
def test_evaluate_token_shards_legacy_shard(tmp_path):
    ...
    with open(tmp_path / "val_shard_0000.bin", "wb") as f:
        f.write(struct.pack("<Q", len(tokens)))
        f.write(arr.tobytes())  # legacy: no dtype_flag
...
def test_evaluate_token_shards_no_tokens_scored(tmp_path):
    """Covers the ValueError when no tokens are scored."""
    from evals.perplexity import evaluate_token_shards
...
# ---------------------------------------------------------------------------
# _load_dataset_texts — TypeError on len() (line 571-573)
# ---------------------------------------------------------------------------

def test_load_dataset_texts_len_type_error():
    """Covers lines 571-573: TypeError when len(dataset) raises."""
    from evals.perplexity import _load_dataset_texts
...
# ---------------------------------------------------------------------------
# evaluate_text_documents — multiple docs, bpb=False path (lines 384-390)
# ---------------------------------------------------------------------------

def test_evaluate_text_documents_multiple_docs():
    """Covers lines 384-390: multiple documents, include_bpb=False."""
    from evals.perplexity import evaluate_text_documents
    ...
    result = evaluate_text_documents(
        model_fn, tokenizer, ["doc one " * 5, "doc two " * 5],
        stride=10, max_length=20, device="cpu",
        include_bpb=False, batch_size=2,
    )
    assert "ppl" in result
