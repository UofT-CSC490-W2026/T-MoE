"""Extra coverage for run_pipeline.py — missing lines 47-50, 62-71, 88-116, 137-162."""
import sys
import pytest
...
def test_load_configuration_import_error_raises():
    """Covers lines 47-50: ImportError path in load_configuration."""
    # Remove infra.config.config from sys.modules to force ImportError
...
def test_load_configuration_value_error_raises():
    """Covers lines 62-71: ValueError path in load_configuration."""
    mock_infra = MagicMock()
    mock_infra.config.config.load_pipeline_config.side_effect = ValueError("missing field")
...
def test_run_sagemaker_pipeline_import_error():
    """Covers lines 88-116: ImportError when sagemaker modules missing."""
    from run_pipeline import run_sagemaker_pipeline
    mock_config = MagicMock()
    # Patch the import inside run_sagemaker_pipeline to raise ImportError
...
def test_run_sagemaker_pipeline_success():
    """Covers lines 88-116: success path of run_sagemaker_pipeline."""
    from run_pipeline import run_sagemaker_pipeline
    mock_config = MagicMock()
    ...
    mock_run_proc.run_processing_job.return_value = "arn:aws:sagemaker:us-east-1:123:job/test"
...
def test_run_sagemaker_pipeline_with_tf_outputs():
    """Covers apply_terraform_outputs branch when tf_outputs is truthy."""
    from run_pipeline import run_sagemaker_pipeline
    mock_config = MagicMock()
    mock_config.dataset_name = "test"
    mock_config.raw_data_bucket = "bucket"
...
def test_run_fallback_pipeline_import_error():
    """Covers lines 137-162: ImportError when fallback_ingestion missing."""
...
def test_run_fallback_pipeline_success():
    """Covers lines 137-162: success path of run_fallback_pipeline."""
    from run_pipeline import run_fallback_pipeline
    mock_config = MagicMock()
    mock_config.dataset_name = "test"
    mock_config.raw_data_bucket = "bucket"
    ...
    with patch.dict(sys.modules, {"infra.data_ingestion.fallback_ingestion": mock_fallback_mod}):
