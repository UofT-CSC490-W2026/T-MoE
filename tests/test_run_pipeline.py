...
def test_main_sagemaker_path():
    ...
    mock_result = {"mode": "sagemaker", "job_arn": "arn:123", "dataset": "test", "s3_bucket": "b"}
    with patch("run_pipeline.load_configuration", return_value=mock_config):
        with patch("run_pipeline.run_sagemaker_pipeline", return_value=mock_result) as mock_sm:
            from run_pipeline import main
            with pytest.raises(SystemExit) as exc_info:
                main()
...
def test_main_fallback_path():
    ...
    mock_config.use_sagemaker = False
    mock_config.dataset_name = "test"
    mock_result = {"mode": "fallback", "dataset": "test", "total_records": 100, "elapsed_seconds": 5.0, "s3_paths": ["a"]}
    with patch("run_pipeline.load_configuration", return_value=mock_config):
        with patch("run_pipeline.run_fallback_pipeline", return_value=mock_result) as mock_fb:
            from run_pipeline import main
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 0
