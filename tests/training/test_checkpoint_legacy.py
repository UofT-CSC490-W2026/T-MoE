...
def test_remap_legacy_moe_state_dict_maps_router_and_lora_keys():
    ...
    assert remapped["backbone.transformer.h.1.mlp.router.gate.weight"] == "router_weight"
    assert (
        remapped["backbone.transformer.h.1.mlp.expert_pool.experts.0.c_fc.lora_A.weight"]
        == "fc1_a"
    )
    assert (
        remapped["backbone.transformer.h.1.mlp.expert_pool.experts.0.c_proj.lora_B.weight"]
        == "fc2_b"
    )
    assert (
        remapped["backbone.transformer.h.3.mlp.expert_pool.experts.2.c_fc.lora_A.weight"]
        == "already_backbone_fc1_a"
    )
    assert (
        remapped["backbone.transformer.h.3.mlp.expert_pool.experts.2.c_proj.lora_B.weight"]
