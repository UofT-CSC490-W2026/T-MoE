...
class FallbackIngestion:
    """Downloads a HuggingFace dataset and uploads it directly to S3."""
    ...
    def run(self) -> Dict[str, Any]:
        """Execute the ingestion pipeline. Returns a summary dict."""
    ...
    def _load_dataset(self) -> Dict[str, Any]:
        """Load dataset from HuggingFace Hub with exponential-backoff retry."""
    ...
    def _get_splits_to_process(self, dataset: Dict[str, Any]) -> List[str]:
        """Return splits to process, validating any explicit requests against available splits."""
    ...
    def _upload_to_s3(self, local_dir: Path, timestamp: str) -> Dict[str, str]:
        """Upload all files in local_dir to S3 under {prefix}/{dataset_name}/{timestamp}/."""
        import sys
        ...
        for local_file in files_to_upload:
            ...
            if s3_client.list_objects(bucket=self.s3_bucket, prefix=s3_key, max_keys=1):
