"""S3 sync utilities for uploading and downloading experiment directories."""
...
def upload_experiment_dir(
    ...
    """Recursively upload a local directory to S3, preserving structure.
    ...
    Args:
        ...
        max_retries: Upload retries per file.

    Returns:
        Dict with 'uploaded' (S3 URIs) and 'failed' (local paths).

    Raises:
        FileNotFoundError: If local_dir does not exist.
        RuntimeError: If the S3 bucket is inaccessible.
    """
    ...
    logger.info("Upload complete: %d succeeded, %d failed", len(uploaded), len(failed))
...
def download_s3_prefix(
    ...
    Preserves key structure relative to s3_prefix.
