---
name: T-MoE Pre-Scale Implementation Plan
overview: ""
todos:
  - id: phase1-delete-dead-code
    content: "Delete 11 dead files: scripts/eval.py, scripts/generate.py, src/training/trainer.py, src/utils/experiment.py, src/utils/logging.py, src/metrics/training_metrics.py, run_pipeline.py, config.yaml, root __init__.py, infra/dataset/ensure_dataset.py, experiments/gptneo_125m_metabolic.yaml"
    status: pending
  - id: phase1-clean-init-imports
    content: Remove dead imports from src/training/__init__.py, src/utils/__init__.py, src/metrics/__init__.py
    status: pending
  - id: phase2a-mmap-sharddataset
    content: "Replace ShardDataset in scripts/train.py: swap np.concatenate for per-shard np.memmap + bisect-based cumulative offset table"
    status: pending
  - id: phase2b-distributed-sampler
    content: "Fix DistributedSampler.set_epoch() in scripts/train.py: track current_epoch, call set_epoch on StopIteration"
    status: pending
  - id: phase2c-aws-container-pipeline
    content: "Fix src/utils/training_workflow.py: add prepare_data subprocess step before training subprocess; add --shard-dir arg to scripts/train.py"
    status: pending
  - id: phase2d-omegaconf-select
    content: "Fix run_aws_training.py line 441: replace attribute access with OmegaConf.select(..., default='/tmp/tmoe_data')"
    status: pending
  - id: phase3a-lr-scheduler
    content: Add warmup + cosine LR scheduler (LambdaLR) to scripts/train.py
    status: pending
  - id: phase3b-checkpoint-manager
    content: Add trainable_only param to CheckpointManager; replace inline save_checkpoint/load_checkpoint in scripts/train.py with CheckpointManager; add periodic save at save_interval
    status: pending
  - id: phase3c-seeds
    content: Add random/numpy/torch seed initialization in scripts/train.py before init_distributed()
    status: pending
  - id: phase4a-modal-retries
    content: Add retries=2 to stage_train in run_modal_training.py
    status: pending
  - id: phase4b-smoketest-rename
    content: Rename experiments/modal_test.yaml to experiments/smoketest.yaml; add DEV-ONLY header; update _DEFAULT_CONFIG in run_modal_training.py
    status: pending
  - id: phase4c-aws-try-finally
    content: Wrap run_container_mode training call in try/finally to always upload checkpoints to S3 even on crash
    status: pending
---
